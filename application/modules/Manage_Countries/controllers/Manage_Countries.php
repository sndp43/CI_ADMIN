<?php
class Manage_Countries extends MX_Controller 
{
...
function manage() 
{
  ...
  //total rows count
  $this->load->model('mdl_manage_Countries');
  $totalRec = count($this->mdl_manage_Countries->getCountries());
  ...
  //get the posts data
  $data["countries"] = $this->mdl_manage_Countries->getCountries(array('limit'=>$this->perPage));
...
function manage_search() {
  ...
  //total rows count
  $this->load->model('mdl_manage_Countries');
  $totalRec = count($this->mdl_manage_Countries->getCountries($conditions));
  ...
  //get posts data
  $data['countries'] = $this->mdl_manage_Countries->getCountries($conditions);
...
{
    $this->load->model('mdl_manage_Countries');
    $query = $this->mdl_manage_Countries->get($order_by);
    return $query;
}
...
function get_with_limit($limit, $offset, $order_by) 
{
    ...
    $this->load->model('mdl_manage_Countries');
    $query = $this->mdl_manage_Countries->get_with_limit($limit, $offset, $order_by);
    return $query;
}
...
function get_where($id)
{
    ...
    $this->load->model('mdl_manage_Countries');
    $query = $this->mdl_manage_Countries->get_where($id);
    return $query;
}
...
{
    $this->load->model('mdl_manage_Countries');
    $query = $this->mdl_manage_Countries->get_where_custom($col, $value);
    return $query;
}
...
{
    $this->load->model('mdl_manage_Countries');
    $this->mdl_manage_Countries->_insert($data);
}
function _insertcountry($data)
{
    $this->load->model('mdl_manage_Countries');
    $this->mdl_manage_Countries->_insertcountry($data);
}
...
function _update($id, $data)
{
    ...
    $this->load->model('mdl_manage_Countries');
    $this->mdl_manage_Countries->_update($id, $data);
}
...
function _delete($id)
{
    ...
    $this->load->model('mdl_manage_Countries');
    $this->mdl_manage_Countries->_delete($id);
}

function count_where($column, $value) 
{
    $this->load->model('mdl_manage_Countries');
    $count = $this->mdl_manage_Countries->count_where($column, $value);
    return $count;
}
...
{
    $this->load->model('mdl_manage_Countries');
    $max_id = $this->mdl_manage_Countries->get_max();
    return $max_id;
}
...
{
    $this->load->model('mdl_manage_Countries');
    $query = $this->mdl_manage_Countries->_custom_query($mysql_query);
    return $query;
}
