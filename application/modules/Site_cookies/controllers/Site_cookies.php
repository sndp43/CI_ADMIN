...
{  
     ...
     $two_weeks_ahead = $nowtime+$two_weeks;
       ...
       $data['cookie_code'] = $this->site_security->GenerateRandomString(128);
       $data['user_id'] = $user_id;
       $data['expiry_date'] = $two_weeks_ahead;
       $this->_insert($data);

       $cookie_name = $this->site_setting->_get_cookie_name();
       $this->_auto_delete_old(); 

setcookie($cookie_name,$data['cookie_code'],$data['expiry_date'], "/"); // 86400 = 1 day
}
...
{   
  ...
  $nowtime = time();
  $mysql_query = "delete from site_cookie where expiry_date<$nowtime"; 
